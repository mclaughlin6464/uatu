...
for epoch in range(epochs):
    ...
    adv_train(model, device, train_dset, optimizer, epoch+1, attack_lr=1e-4)#, smoothing = 1)
    val_test(model, device, val_dset)

    if epoch%10==0:
        torch.save(model.state_dict(), path.join(output_dir, 'deep_resnet_quijote_%02d_adv.pth'%(epoch)))
