...
model_path = path.join(output_dir, 'gupta_net_smooth_%0.1f_noise_%0.2f_epoch_%02d_adv.pth'%(smooth, noise,epoch))
...
output_fname  = path.join(dir, 'UatuFastPMTrainingRobustifyGuptaNetAdvGRF.hdf5')
...
# TODO start batches
    ...
    robust_x = compute_robust_map(scattering, device, model, x0, xt, get_embedding=get_gupta_embedding).cpu().detach().numpy()
