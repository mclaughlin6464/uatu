...
init_epoch = 7
if init_epoch>0:
    model_path = path.join(output_dir, 'deep_resnet_reg_smooth_%0.1f_noise_%0.1f_%02d_adv2.pth'%(smooth, noise,init_epoch))
...
for epoch in range(init_epoch, epochs):
    ...
    adv_train(model, device, orig_train_dset, optimizer, epoch+1)#, smoothing = 1)
    #val_test(model, device, val_dset)
    val_test(model, device, orig_val_dset)

    if epoch%1==0:
        torch.save(model.state_dict(), path.join(output_dir, 'deep_resnet_reg_smooth_%0.1f_noise_%0.1f_%02d_adv2.pth'%(smooth, noise, epoch)))
