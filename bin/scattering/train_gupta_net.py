...
orig_train_dset = DatasetFromFile(orig_fname,batch_size, shuffle=True, augment=True, train_test_split = 0.7,\
                                 whiten = False, cache_size = 100, data_mod=data_mod, transform=transform)
...
for epoch in range(init_epoch, epochs):
    ...
    if epoch%1==0:
        torch.save(model.state_dict(), path.join(output_dir, 'gupta_net_reg_smooth_%0.1f_noise_%0.2f_epoch_%02d.pth'%(smooth, noise, epoch)))
