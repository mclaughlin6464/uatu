...
clone_fname  = path.join(dir, 'UatuFastPMTrainingRobustifyDeepResnetAdvGRFNoise%d.hdf5'%int(smooth))
...
train_dset = DatasetFromFile(clone_fname,batch_size, shuffle=True, augment=True, train_test_split = 0.7,\
                                 whiten = False, cache_size = 200, data_mod=lambda x:x, transform=transform)
...
init_epoch = 0
output_fname = 'deep_resnet_reg_smooth_%0.1f_noise_%0.1f_%02d_clone.pth'
if init_epoch>0:
...
for epoch in range(init_epoch, epochs):
    ...
    adv_train(model, device, train_dset, optimizer, init_epoch+epoch+1)#, smoothing = 1)
    val_test(model, device, val_dset)
    val_test(model, device, orig_val_dset)

    if epoch%1==0:
        torch.save(model.state_dict(), path.join(output_dir, output_fname%(smooth, noise,init_epoch+epoch)))
