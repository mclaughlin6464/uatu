...
orig_fname = path.join(dir, 'UatuLightconeTraining.hdf5')
clone_fname = path.join(dir, 'UatuLightconeTrainingAdvGuptaRobustified1.hdf5')
...
orig_train_dset = DatasetFromFile(orig_fname,batch_size, shuffle=True, augment=True, train_test_split = 0.8,\
                                 whiten = True, cache_size = 100, transform=transform)
...
#        lr = 5e-7
    ...
    train(model, device, train_dset, optimizer, epoch+1, print_every=200, loss = 'mae')
    val_test(model, device, val_dset)
    val_test(model, device, orig_val_dset)

    if epoch%1==0:
        torch.save(model.state_dict(), path.join(output_dir, 'gupta_net_adv_clone_epoch_%02d.pth'%(epoch)))
