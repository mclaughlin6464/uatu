...
train_dset = Dataset(X_train,y_train, 1, shuffle=True, augment=True)
#val_dset = Dataset(X_val,y_val, 30, shuffle=True, augment=True)
test_dset = Dataset(X_test, y_test, 1, shuffle=True, augment=True)
...
#device = '/cpu:0'
print time() - t0
train(shallow_bayesian_convnet_init_fn, standard_optimizer_init_fn, bayes_cost_fn, data, device, num_epochs = 1, fname = './tmp_bayes', print_every = 1, lr = 0.0005) 
