...
fname = path.join(dir, 'UatuLightconeTest_ShuffleNorm.hdf5')

test_dset = DatasetFromFile(fname, batch_size=1, shuffle = False, augment = False, whiten = True, train_test_split = 1.0, cache_size = 1, y_key = 'true_Y')

test(gupta_network_init_fn, test_dset, n_samples = 1 ,\
        fname = '/home/users/swmclau2/scratch/uatu_networks/gupta_net_kappa_abs_shuffle_attack_v2-21000',\
     #, samples_fname = path.join(dir, 'UatuLightconePredsRenorm.hdf5'))
     samples_fname = path.join(dir, 'UatuLightconePredsShuffleAttack_v2.hdf5'))
