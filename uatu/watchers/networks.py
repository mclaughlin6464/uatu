...
def standard_convnet_init_fn(inputs, training= False):
    ...
    prob = 0.5 if training else 1.0 #should i do some fancier tf stuff?
    # NOTE ask waren if i need separate relus
    layers = [ tf.layers.Conv3D(2, kernel_size=(62, 62, 62), padding='same',
                                  kernel_initializer=initializer),
               tf.layers.BatchNormalization(axis=1,
                                            gamma_initializer=initializer),
               tf.keras.layers.LeakyReLU(alpha=0.01),
               tf.layers.AveragePooling3D(pool_size=(31, 31, 31)),
               ...
               tf.layers.BatchNormalization(axis=1,
                                            gamma_initializer=initializer),
               tf.keras.layers.LeakyReLU(alpha=0.01),
               tf.layers.AveragePooling3D(pool_size=(14, 14, 14)),
