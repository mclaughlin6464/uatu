...
def standard_convnet_init_fn(inputs, training= False):
    ...
    #ap1_out = tf.layers.average_pooling2D(lr1_out, pool_size=(31, 31, 31), strides = 2)
    conv2_out = tf.layers.conv2d(lr1_out, 32, kernel_size=5, padding='same')
    #                              kernel_initializer=initializer)
    ...
    conv3_out = tf.layers.conv2d(lr2_out, 64, kernel_size=5, padding='same')
    #                  kernel_initializer=initializer)
    ...
    ap1_out = tf.layers.average_pooling2d(lr6_out, pool_size=3, strides = 2)
